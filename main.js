...
(function () {
  ...
  function tick() {
    ...
    // Dừng lại đúng lúc chạm mốc 50%, đợi thêm ~2.5 giây trước khi hỏi bật nhạc
    // (giữ đúng nhịp trải nghiệm như bản cũ)
...
/* ─── Background Music (local MP3 file) ─────────────────────────────── */
// Đặt file nhạc tên "music.mp3" cùng thư mục với index.html (thẻ <audio>
// trong HTML đã trỏ tới file này sẵn). Muốn đổi tên file thì sửa thuộc
// tính src của thẻ #bg-music trong index.html.
...
window.startBackgroundMusic = function () {
  ...
  if (playPromise && playPromise.catch) {
    playPromise.catch(function () {
      // Trình duyệt chặn autoplay có âm thanh: thử lại khi người dùng
      // tương tác lần đầu với trang.
      var retry = function () {
...
/* ─── Gạch Thẻ: Calculator Demo ──────────────────────────────────────── */
(function () {
  const denomBtns = document.querySelectorAll('.card-calc-denom');
  const qtyInput  = document.getElementById('card-calc-qty');
  const totalEl   = document.getElementById('card-calc-total');
  const feeEl     = document.getElementById('card-calc-fee');
  const finalEl   = document.getElementById('card-calc-final');
  if (!denomBtns.length || !qtyInput) return;
  ...
  function recalc() {
    const active = document.querySelector('.card-calc-denom.active');
    const value  = active ? parseInt(active.dataset.value, 10) : 0;
    ...
    feeEl.textContent   = formatVND(fee);
    finalEl.textContent = formatVND(final);
  }
